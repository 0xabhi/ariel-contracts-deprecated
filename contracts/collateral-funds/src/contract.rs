...
pub fn deposit(deps: DepsMut, info: MessageInfo) -> Result<Response, ContractError> {
    ...
    Ok(Response::new().add_attribute("method", "deposit_collateral").add_attribute("amount", info.funds[0].amount))
}
...
pub fn withdraw(deps: DepsMut, info: MessageInfo, to: Addr, amount: u128) -> Result<Response, ContractError> {
    ...
    Ok(Response::new().add_message(send_tx_msg).add_attribute("method", "withdraw_collateral").add_attribute("amount", amount))
}
